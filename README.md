...
- 사용자 키워드 기반 멜로디 생성(키워드를 통해 곡의 템포와 느낌, 주제 등을 사용자 니즈에 따라 설정할 수 있음)
...
3. 환경 변수 설정
- 프로젝트 루트 디렉토리에 `.env` 파일을 생성하고 OpenAI API 키를 설정합니다. 프로젝트 공개 및 실행을 위해 특별히 제 계정의 API 키를 한시적으로 공개합니다. 아래 이미지와 같이 .env 파일에 키를 그대로 입력하세요.
...
- 현재 공개한 API 키는 프로젝트 시연 및 데모 목적으로만 제공합니다. API 키를 이 저장소 외 다른 환경에 직접 노출하지 않도록 주의하세요.
- GPT-4 API는 사용량 제한이 있습니다. (현재 유료버전 사용중)
- 이 프로젝트는 가급적 최신 버전의 크롬 브라우저 환경에서 실행하기를 권장합니다.
